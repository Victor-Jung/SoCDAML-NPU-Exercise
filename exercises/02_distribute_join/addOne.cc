...
//
// Two entry points are exported (addOneSmall, addOneLarge) so that MLIR
// can declare separate function signatures for different buffer sizes.
// Both call the same vectorized implementation.
//
//===----------------------------------------------------------------------===//
...
static void addOneImpl(int32_t *restrict in, int32_t *restrict out,
                       int32_t lineWidth) {
  event0();
...
void addOneSmall(int32_t *restrict in, int32_t *restrict out,
                 int32_t lineWidth) {
  addOneImpl(in, out, lineWidth);
}

void addOneLarge(int32_t *restrict in, int32_t *restrict out,
                 int32_t lineWidth) {
  addOneImpl(in, out, lineWidth);
}
